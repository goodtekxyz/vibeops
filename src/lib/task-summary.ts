 ...
 *
 * We never include placeholder content such as `(미수행)` or `(scaffolded …)`
 * — those become an empty string.
 */
...
const PLACEHOLDER_RE = /^\(.*(미수행|미정|scaffold|fill in|채워라).*\)$/i;
...
export function renderPulledTaskMarkdown(inputs: PulledTaskInputs): string {
  ...
  const phase = inputs.mvpPhase.length > 0 ? inputs.mvpPhase : "(미정)";
  const summary = inputs.summary.length > 0 ? inputs.summary : "(Notion에 Summary가 비어 있다 — 채워라.)";
...
> 이 파일은 \`vibeops task pull\`이 Notion에서 가져온 메타로 만들어졌다. 본문은 비어 있으니 Builder/Planner Agent가 채워야 한다.
...
(pulled — Notion Summary 이외 정보는 여기서 채워라.)
...
1. (pulled — 검증 가능한 문장 1)
2. (pulled — 검증 가능한 문장 2)
...
(시작 시 \`vibeops task start ${inputs.taskId}\`이 채움)
