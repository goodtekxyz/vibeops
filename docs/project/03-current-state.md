...
- **현재 단계**: MVP 1~4 구현 + 패키지 마무리 + GitHub 연동 + Init Git bootstrap UX + Status output polish + Notion env template cleanup + Public release polish + **Template English localization** **Review 대기**.
  ...
  - Template English localization — **TASK-018 (`templates/**` 전체 영문화 + `.vibeops/` 자체 미러 sync) Review 대기**.
...
| TASK 목록                      | `docs/tasks/TASK-001 ~ TASK-018`                | TASK-001~006 done, **TASK-007~018 Review 대기**                       |
...
| **템플릿 콘텐츠 (36개)**       | `templates/**`                                  | AGENTS.md / 5 rules / 8 agents / 6 prompts / 4 workflows / 10 project docs / TASK-000 / logs README. **TASK-018** 로 전부 영문화 (placeholders / paths / section headings / VibeOps 용어 보존). `.vibeops/agents` · `.vibeops/prompts` · `.vibeops/workflows` 18 파일을 templates 와 1:1 sync. sandbox `vibeops init --git --initial-commit` 후 사용자 워크스페이스의 한국어 글자 0건. |
